# Flightrec — Build-in-Public Playbook (X / Twitter)
...
## Daily template
```
day {N} of building Flightrec 🦋⏪ (open-source time-travel debugger for Next.js)
...
- "Hot take: most Next.js debugging pain is that these 6 planes live in 6 different tools. Flightrec puts them on one scrubber. Here's the architecture 🧵"

### Phase 1 — Landing page + demo (BIG moment — launch tweet)
- LAUNCH: "Introducing Flightrec ⏪ — a time-travel debugger for Next.js App Router. Rewind a session and see exactly which Server Action ran, which cache tags changed, what RSC streamed, and how the client tree reacted. All on one timeline. Live demo + open source 👇"
...
- "Flightrec can now load a real `.frec` trace bundle and render it — drag, drop, scrub. Export from one machine, open on another. [clip]"
...
- "It's real. Flightrec now *records* an actual Next.js session via `instrumentation.ts` — Server Actions, cache tags, RSC frames, the client tree — and replays it tick by tick. No shadow React runtime, just a deterministic replay graph. [clip]"
...
- "Most tools tell you 'tag invalidated'. Flightrec tells you whether the user actually saw fresh data — and classifies it: immediate-freshness / stale-then-refresh / no-visible-effect / orphaned-invalidation. That last one is a bug you didn't know you had. [clip]"
...
- "Flightrec now ships an MCP server. Ask your coding agent: 'which Server Action caused this stale state?' and it queries the actual session trace. [clip]"
